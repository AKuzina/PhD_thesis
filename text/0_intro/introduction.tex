...
For example, an invertible transformation in the Normalizing Flows or Score function in Score-based model is usually a specifically parametrized Neural Network, which we denoted as $f_{\theta}$ in Eq.~\ref{eq:intro_nf} and Eq.~\ref{eq:intro_score}. Due to neural networks flexibility, this allows us to obtain very flexibly probility distributions. 

In autoregressive and latent variable models, one the other hand, we usually assume an "easy to work with" conditional distributions, like Gaussian. However, we use neural networks to predict the parameters of those distributions. 
For instance, consider a conditional distribution $p_{\theta}(\rvx|\rvz)$. Here, we would usually assume that there is a function $f_{\theta}$, which maps a value of $\rvz$ (e.g. a realization of the random variable $\rvz$) into parameters of the probability distribution $p_{\theta}(\rvx|\rvz)$. For example, if $\rvx$ is a continuous random variable with infinite support, one can use gaussian distribution:
\begin{equation}
    p_{\theta}(\rvx|\rvz) = \mathcal{N}(\rvx| \mu_{\theta}(\rvz), \sigma^2_{\theta}(\rvz) \text{I}),
\end{equation}
where both $\mu_{\theta}(\cdot)$ and $\sigma^2_{\theta}(\cdot)$ are Neural Networks (NNs) and $\theta$ are the trainable parameters of these NNs. Other conditional distributions that are commonly used are Bernoulli, Discretized Logistic and Categorical. \todo{Shall I write formula for discretized logistic distribution}
...
In this Section, we will discuss gradient-based optimization used throughout the thesis and derive several objective functions for deep generative models. 

\subsection{Stochastic Gradient Descent}
Consider a loss function $\mathcal{L}(\rvx, \theta)$. 
\marginnote[-10pt]{A function that should be minimized with respect to model parameters will be called \textit{loss function}, \textit{cost function} and \textit{objective} interchangeably.}
Assume also that we can also compute the gradient $\nabla_\theta \mathcal{\rvx, \theta}$. Note that this is computed for a single datapoint, while we observe the dataset, containing $N$ independent points, there for the expected loss that we want to minimize is the following:
\begin{equation}\label{eq:intro_loss}
\E_{p_e(\rvx)} \mathcal{L}(\rvx, \theta) = \frac1N \sum_n \mathcal{L}(\rvx_n, \theta) \rightarrow \min_{\theta}.
\end{equation}

We can then apply \textit{gradient descent}, a first-order iterative optimization algorithm. Under certain mild conditions on the loss function (e.g. it is convex), this algorithm convergence to the optimum. At each iteration of the method, we make a step in the direction of the fastest decrease of the loss function. We can also adjust the step size with a hyperparameter called \textit{learning rate}, that we will denote as $\eta_t$.
...
\State \hskip-3mm  {\bfseries Input:} { Loss function $\mathcal{L}(\rvx, \theta)$}
	...
	\end{algorithmic}
\end{algorithm}
\marginnote[-10\baselineskip]{We consider standard SGD update rule. Other first-order optimization methods (e.g. Adam~\citep{kingma2015adam}) use momentum to adjust the noisy gradient.}
...
\end{aligned}
\end{equation}
where we used definition of KL divergence and the fact that entropy of the empirical data distribution does not depend on the parameters $\theta$. Thus, maximum likelihood objective:
\begin{equation}
...
\end{align}
% \end{fullwidth}
where Eq.~\ref{eq:sm_2} is the result of opening the brackets and noticing that the last term does not depend on the parameters $\theta$ and integration by parts is applied to the second term of the Eq.~\ref{eq:sm_2} to obtain Eq.~\ref{eq:sm_3}. As a result, we obtain the following \textit{score matching} objective:
...
\begin{equation}
\begin{aligned}\label{eq:intro_elbo_def}
    \log p_{\theta}(\rvx) &=  \log \int p_{\theta}(\rvx|\rvz)p_{\theta}(\rvz)d\rvz \\
   &=  \log \int \frac{q(\rvz)}{q(\rvz)} p_{\theta}(\rvx|\rvz)p_{\theta}(\rvz)d\rvz \\
   &=  \log \E_{q(\rvz)} \frac{p_{\theta}(\rvx|\rvz)p_{\theta}(\rvz)}{q(\rvz)} \\
   &\geq  \E_{q(\rvz)}\log  \frac{p_{\theta}(\rvx|\rvz)p_{\theta}(\rvz)}{q(\rvz)} \\
   &\overset{def}{=} \mathcal{F}(\rvx, q, \theta), \\
\end{aligned}
\end{equation}
where we have applied Jensen's inequality\footnote{For a random variable $\rvx$ and a convex function $f$ it holds that $f(\E[\rvx]) \leq \E[f(\rvx)]$.}. 
This is a variational lower bound, which is also known as Evidence Lower BOund (ELBO) with \textit{variational} parameter $q$. 
ELBO can be more tight or less tight depending on this variational distribution $q$.
...
Expectation Maximization Algorithm, also known as a EM-algorithm~\citep{dempster1977maximum} is an iterative algorithm to infer posterior over the latent variables and maximum likelihood estimate of the parameters for a class of problems, where $p_{\theta}(\rvz|\rvx)$ is tractable if we fix parameters of the generative model $\theta$. At each iteration, 

\begin{equation}
\begin{aligned}
    \text{\textbf{E-step:  }} &q^{t+1}_n(\rvz) = \max_{q_n} \mathcal{F}(\rvx_n, q_n, \theta) = p_{\theta^t}(\rvz|\rvx_n),\,\forall n \in \{1\dots N\}\\
    \text{\textbf{M-step:  }} &\theta^{t+1} = \max_{\theta} \E_{p_e(\rvx)}\mathcal{F}(\rvx, q^{t+1}_n, \theta)\\
\end{aligned}
\end{equation}

From this formulation, we see that both steps of the EM-algorithm solve the same optimization problem: maximizing the ELBO. Expectation step computes posterior distribution given the model parameters from the previous step. Maximization step uses this posterior to optimize the ELBO with respect to parameters $\theta$.

This observation allows us to generalize this to a wider class of problems. For example, if we assume that $\rvz$ is a continious random variable, 
...
Analogously to the stochastic version of the gradient descent, stochastic Variational Inference (SVI) was proposed~\citep{hoffman2013stochastic}, where at each iteration only a subset of data is used, and thus only a subset of the local variational parameters $\phi_n$ is updated. 
...
This decomposition highlights the importance of multimodal priors. If we take a closer look at the first term, it i
...
\begin{tikzpicture}
  ...
  name=plot1,
    ...
    \addplot[blue, thick] {1/(sqrt(2*pi*0.75)) *  exp(-0.5 * (x-5)^2/0.75};
  \end{axis}
