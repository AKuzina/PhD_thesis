...
Let us introduce an \textit{ aggregated posterior} which is defined as a variational posterior averaged over the entire dataset: $q_{\phi}(\rvz) = \E_{p_{e}(\rvx)}q_{\phi}(\rvz|\rvx)$. Consider an illustrative example in Figure~\ref{fig:intro_prior_mismatch}, where we plot a standard Gaussian prior and an aggregated posterior which is a multimodal distribution. We highlight a clear hole in the latent space with the red circle. This motivated the use of more complicated prior distributions, which are capable of \textit{adapting} to the aggragated posterior and are flexible enough to avoid the holes as much as possible. 
...
\begin{axis}[
    ...
    title={\normalsize Prior},
    axis lines=none,
    colormap={greenmap}{color(0cm)=(ForestGreen!10); color(1cm)=(ForestGreen!99)},
...
{exp(-0.6*(x^2 + y^2)};
\draw[very thick, RedOrange] (-0.4,0) circle (0.3cm); 
...
\end{marginfigure}
\todo[inline]{Flow prior and Maybe other ones}
...
\todo[inline]{If prior is data-dependent it can be used to store information and avoid catastrophic forgetting in continual learning.}
